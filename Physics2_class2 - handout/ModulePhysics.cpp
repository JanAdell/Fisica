...
update_status ModulePhysics::PostUpdate()
{
	...
	if(App->input->GetKey(SDL_SCANCODE_2) == KEY_DOWN)
	{
		...
		myBodyDef.position.Set(PIXEL_TO_METERS(App->input->GetMouseX()), PIXEL_TO_METERS(App->input->GetMouseY()));
		...
		b2Body* c = world->CreateBody(&bodyDef);
		b2FixtureDef fix;
		c->CreateFixture(&fix);		b2Vec2 vertices[4];
	...
	if(App->input->GetKey(SDL_SCANCODE_3) == KEY_DOWN)
	{
		...
		b2BodyDef letter;
		letter.position.Set(PIXEL_TO_METERS(App->input->GetMouseX()), PIXEL_TO_METERS(App->input->GetMouseY()));
		letter.angle = 0; //set the starting angle

		b2Body* Body = world->CreateBody(&letter);
